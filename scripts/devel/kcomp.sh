...
--without-* can be used to remove features at runtime (TODO).
...
#   if [ "${HAVE_MODULES_BUILT:-n}" != "y" ]; then
#      autodie kcomp_make modules
#      HAVE_MODULES_BUILT=y
#   fi
   # let's cheat
   touch "${KBUILD}/Module.symvers"
...
argparse_longopt() {
   case "${longopt}" in
      ...
      ;;
      'with-'*)
         local flag="${longopt#with-}"
         case "${flag}" in
            vbox)
               enable_use "virtualbox"
            ;;
            *-*)
               # dash doesn't support ${var//-/_}
               enable_use `echo "${flag}" | sed -e 's,-,_,g'`
            ;;
            *)
               enable_use "${flag}"
            ;;
