#!/bin/sh
#
# *** REPLACED by buildvars.sh + dobuild-ng ***
#
...
USE="${USE-}${USE:+ }\${USE-}"
[ -n "\${USE}" ] && [ "\${USE_REDUX:-y}" = "y" ] || readonly USE

TARGET_SHLIB_ROOT="${TARGET_SHLIB_ROOT:-/sh}"
TARGET_SHLIB_NAME="${TARGET_SHLIB_NAME:-shlib.sh}"
...
: \${DEFAULT_SCRIPT_INTERPRETER=${SCRIPT_INTERPRETER:-/bin/sh}}

. "${B_ENV:?}" || exit

if [ -n "\${USE-}" ] && [ "\${USE_REDUX:-y}" = "y" ]; then
   list_redux \${USE}
   readonly USE="\${v0}"
   unset v0
fi
...
BUILD_ROOT=$(readlink -m "${2}" 2>/dev/null || readlink -f "${2}")
readlink -f "${2}"
...
#
if \
   [ "${DOBUILD_INTERPRETER:-/bin/sh}" = "/bin/sh" ] && \
   { [ -n "${BASH_VERSION-}" ] || [ "x${SHELL-}" = "x/bin/bash" ]; }
then
   bash ${B_SCRIPT} "$@"
else
   unset SHELL
